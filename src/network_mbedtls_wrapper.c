...
static int _iot_tls_verify_cert(void *data, mbedtls_x509_crt *crt, int depth, uint32_t *flags) {
    ...
    pr_debug("Verify requested for (Depth %d):", depth);
    mbedtls_x509_crt_info(buf, sizeof(buf) - 1, "", crt);
    pr_debug("%s", buf);

    if((*flags) == 0) {
        pr_debug("  This certificate has no flags");
    } else {
        pr_debug("Verify result:%s", buf);
    }
...
IoT_Error_t iot_tls_connect(Network *pNetwork, TLSConnectParams *params) {
    ...
    pr_debug("Seeding the random number generator...");
    ...
    if((ret = mbedtls_ctr_drbg_seed(&(tlsDataParams->ctr_drbg), mbedtls_entropy_func, &(tlsDataParams->entropy),
        ...
        pr_debug("failed! mbedtls_ctr_drbg_seed returned -0x%x", -ret);
...
//        ret = mbedtls_x509_crt_parse_file(&(tlsDataParams->cacert), pNetwork->tlsConnectParams.pRootCALocation);
    ...
    if(ret < 0) {
        pr_debug("failed!  mbedtls_x509_crt_parse returned -0x%x while parsing root cert", -ret);
        return NETWORK_X509_ROOT_CRT_PARSE_ERROR;
    }
    pr_debug("ok (%d skipped)", ret);

    /* Load client certificate... */
    if (pNetwork->tlsConnectParams.pDeviceCertLocation[0] == '/') {
        pr_debug("Loading client cert from file...");
//        ret = mbedtls_x509_crt_parse_file(&(tlsDataParams->clicert),
//                                          pNetwork->tlsConnectParams.pDeviceCertLocation);
    } else {
        pr_debug("Loading embedded client certificate...");
    ...
    if(ret != 0) {
        pr_debug("failed!  mbedtls_x509_crt_parse returned -0x%x while parsing device cert", -ret);
        return NETWORK_X509_DEVICE_CRT_PARSE_ERROR;
    }

    /* Parse client private key... */
    if (pNetwork->tlsConnectParams.pDevicePrivateKeyLocation[0] == '/') {
        pr_debug("Loading client private key from file...");
//        ret = mbedtls_pk_parse_keyfile(&(tlsDataParams->pkey),
//                                       pNetwork->tlsConnectParams.pDevicePrivateKeyLocation,
//                                       "");
    } else {
        pr_debug("Loading embedded client private key...");
    ...
    if(ret != 0) {
        pr_debug("failed!  mbedtls_pk_parse_key returned -0x%x while parsing private key", -ret);
    ...
    pr_debug("Connecting to %s/%s...", pNetwork->tlsConnectParams.pDestinationURL, portBuffer);
    if((ret = mbedtls_net_connect(&(tlsDataParams->server_fd), pNetwork->tlsConnectParams.pDestinationURL,
        ...
        pr_debug("failed! mbedtls_net_connect returned -0x%x", -ret);
        switch(ret) {
    ...
    if(ret != 0) {
        pr_debug("failed! net_set_(non)block() returned -0x%x", -ret);
        return SSL_CONNECTION_ERROR;
    } 
    pr_debug("ok");

    pr_debug("Setting up the SSL/TLS structure...");
    if((ret = mbedtls_ssl_config_defaults(&(tlsDataParams->conf), MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM,
                                          MBEDTLS_SSL_PRESET_DEFAULT)) != 0) {
        pr_debug("failed! mbedtls_ssl_config_defaults returned -0x%x", -ret);
        return SSL_CONNECTION_ERROR;
    }
    ...
    if(ret != 0) {
        pr_debug("failed! mbedtls_ssl_conf_own_cert returned %d", ret);
        return SSL_CONNECTION_ERROR;
    }
    ...
    if((ret = mbedtls_ssl_setup(&(tlsDataParams->ssl), &(tlsDataParams->conf))) != 0) {
        pr_debug("failed! mbedtls_ssl_setup returned -0x%x", -ret);
        return SSL_CONNECTION_ERROR;
    }
    if((ret = mbedtls_ssl_set_hostname(&(tlsDataParams->ssl), pNetwork->tlsConnectParams.pDestinationURL)) != 0) {
        pr_debug("failed! mbedtls_ssl_set_hostname returned %d", ret);
        return SSL_CONNECTION_ERROR;
    }
    pr_debug("SSL state connect : %d ", tlsDataParams->ssl.state);
    mbedtls_ssl_set_bio(&(tlsDataParams->ssl), &(tlsDataParams->server_fd), mbedtls_net_send, NULL,
                        mbedtls_net_recv_timeout);
    pr_debug("ok");

    pr_debug("SSL state connect : %d ", tlsDataParams->ssl.state);
    pr_debug("Performing the SSL/TLS handshake...");
    while((ret = mbedtls_ssl_handshake(&(tlsDataParams->ssl))) != 0) {
        if(ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
            pr_debug("failed! mbedtls_ssl_handshake returned -0x%x", -ret);
            if(ret == MBEDTLS_ERR_X509_CERT_VERIFY_FAILED) {
                pr_debug("    Unable to verify the server's certificate. ");
            }
            return SSL_CONNECTION_ERROR;
        }
    }

    pr_debug("ok    [ Protocol is %s ]    [ Ciphersuite is %s ]", mbedtls_ssl_get_version(&(tlsDataParams->ssl)),
          mbedtls_ssl_get_ciphersuite(&(tlsDataParams->ssl)));
    if((ret = mbedtls_ssl_get_record_expansion(&(tlsDataParams->ssl))) >= 0) {
        pr_debug("    [ Record expansion is %d ]", ret);
    } else {
        pr_debug("    [ Record expansion is unknown (compression) ]");
    }

    pr_debug("Verifying peer X.509 certificate...");
    ...
    if(pNetwork->tlsConnectParams.ServerVerificationFlag == true) {
        ...
        pr_debug(" Server Verification skipped");
        ret = SUCCESS;
    }

//    if(LOG_LOCAL_LEVEL >= ESP_LOG_DEBUG) {
        if (mbedtls_ssl_get_peer_cert(&(tlsDataParams->ssl)) != NULL) {
            pr_debug("Peer certificate information:");
...
IoT_Error_t iot_tls_write(Network *pNetwork, unsigned char *pMsg, size_t len, Timer *timer, size_t *written_len) {
    ...
    for(written_so_far = 0, frags = 0;
        ...
        while(!has_timer_expired(timer) &&
            ...
            if(ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
                pr_debug("failed! mbedtls_ssl_write returned -0x%x", -ret);
